...
class Rhel(Distro):
    ...
    def createKickstart(self):
        ...
        if vmParser.args.install_protocol == 'ftp':
            if version.startswith('8') or  version.startswith('9'):
                lstr = "%end"
                urlstring = "--url=ftp://"+vmParser.confparser('ftpRepo', 'RepoIP') + ':' + vmParser.confparser('ftpRepo', 'FtpRepoDir')+ "/BaseOS"
            else:
                lstr = "telnet\njava\n%end"
                urlstring = "--url=ftp://"+vmParser.confparser('ftpRepo', 'RepoIP') + ':' + vmParser.confparser('ftpRepo', 'FtpRepoPort') + self.FtpRepoDir
        if vmParser.args.install_protocol == 'nfs':
            if version.startswith('8') or  version.startswith('9'):
                ...
                lstr = "telnet\njava\n%end"
                urlstring = "--url=nfs://"+vmParser.confparser('nfsRepo', 'RepoIP') + ':' + vmParser.confparser('nfsRepo', 'NfsRepoDir') 
    ...
    def configGrub(self):
        ...
        if vmParser.args.install_protocol == 'ftp':
            strLnx = '    linux ' + vmParser.netDir + '/ppc/ppc64/vmlinuz ' + cli_nw + \
                 ' inst.repo=ftp://' + vmParser.confparser('repo', 'RepoIP') + \
                   vmParser.confparser('ftpRepo', 'FtpRepoDir') + \
